> Historical workspace notes from the initial build. For current public installation and release status, read [the project README](../README.md) and [CLI guide](../packages/cli/README.md). Some UI descriptions and installation limitations below reflect earlier checkpoints.
...
The browser nests items under their category and shows preview and source alongside each other on desktop. Research inventories: [first-party registry candidates](first-party-registry-audit.md) and [JSON UI formats](json-ui-formats-research.md). Vue demos currently require the configured sibling ai-elements-vue checkout.
