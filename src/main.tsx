...
function App({items}:{items:Item[]}) {
 ...
 {demo.kind==='App'?<Tabs key={demo.id} defaultValue="preview" className="app-workbench"><div className="app-tabbar"><TabsList aria-label="App view"><TabsTrigger value="preview">Preview</TabsTrigger><TabsTrigger value="code">Code</TabsTrigger></TabsList></div><TabsContent value="preview" forceMount className="app-preview-panel"><div className="demo-canvas"><DemoFrame key={`${demo.id}:${revision}`} id={demo.id} title={group.name} width={width} previewProps={previewProps} viewport/></div></TabsContent><TabsContent value="code" className="app-code-panel"><ComponentReference demo={demo} family={group.name} onConfigure={setPreviewProps}/></TabsContent></Tabs>:<div className="workbench-content"><div className="demo-canvas"><DemoFrame key={`${demo.id}:${revision}`} id={demo.id} title={group.name} width={width} previewProps={previewProps} viewport={demo.kind==='Block'}/></div>
